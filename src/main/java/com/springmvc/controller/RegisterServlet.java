...
public class RegisterServlet extends HttpServlet{
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (rank.equals("3")){
            ...
            if (rank2.equals("1") || rank2.equals("2")){
                ...
                browser.id = Integer.parseInt(id);
                User user = new User();
                if ((role.ifroot == 1) && password.equals("") == false){
                    provinceTable table = new provinceTable();
                    user.accompanyName = province.accompanyName;
                    user.password = password;
                    boolean end2 = table.insert(user);
                    if (end2){
                        table.find("accompanyName", user.accompanyName, user);
                        Role role1 = new Role();
                        role1.id = user.id;
                        role1.SetDate = setDate;
                        role1.SearchCompany = searchCompany;
                        role1.SearchData = searchData;
                        role1.SetUser = setUser;
                        role1.CreateNews = createNews;
                        role1.SetSystem = setSystem;
                        role1.SetRole = 0;
                        role1.ifroot = 0;
                        ...
                        browser.content = browser.content + "注册了一个新角色";
                        browserTable table1 = new browserTable();
                        table1.insert(browser);
                        out.print("新用户账户为: " + user.account);
                    }else {
